		...
		<div class="al-box filter">
			...
			<el-table :ref="tableRef" :data="tableData" style="width: 100%;" size="large"
				...
				<el-table-column label="分类" align="center">
					<template #default="scope">
						<el-tag type="warning">{{scope.row.c_name || '无'}}</el-tag>
					</template>
				</el-table-column>
		...
		<el-dialog v-model="dialogFormVisible" :title="isEdit ? '编辑产品' : '新增产品'" destroy-on-close>
			<el-form :model="tableForm" :label-width="80" size="large">
				...
				<el-form-item label="分类：">
					<el-select v-model="tableForm.c_id" placeholder="请选择">
...
<script setup>
	...
	import {apiProduct,apiProductCateGory} from '@/api/product.js'
	...
	const getList = () => {
		...
		apiProductCateGory({limit:999},'get').then(res=>{
